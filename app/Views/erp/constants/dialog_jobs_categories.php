...
$get_animate = '';
if($request->getGet('data') === 'jobs_categories' && $request->getGet('field_id')){
$category_id = udecode($field_id);
$result = $ConstantsModel->where('constants_id', $category_id)->where('type','jobs_categories')->first();
?>
			...
			success: function (JSON) {
				if (JSON.error != '') {
					...
					var xin_table = $('#xin_table').dataTable({
						"bDestroy": true,
						"ajax": {
							url : "<?= site_url("erp/types/jobs_categories_list") ?>",
							type : 'GET'
						},
