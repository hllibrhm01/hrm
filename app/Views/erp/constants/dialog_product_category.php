...
$result = $ConstantsModel->where('constants_id', $category_id)->where('type','product_category')->first();
?>

<div class="modal-header">
  <h5 class="modal-title">
    <?= lang('Main.xin_edit_product_category');?>
			...
			success: function (JSON) {
				if (JSON.error != '') {
					...
					var xin_table = $('#xin_table').dataTable({
						"bDestroy": true,
						"ajax": {
							url : "<?= site_url("erp/types/products_category_list") ?>",
							type : 'GET'
						},
